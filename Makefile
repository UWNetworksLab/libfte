...
ARFLAGS = rsc
ifeq ($(EMSCRIPTEN),1)
#OPTIMIZATION_FLAGS = -g0 --closure 1 -O3
OPTIMIZATION_FLAGS = -O0
CFLAGS_ = $(CFLAGS) $(OPTIMIZATION_FLAGS) -Wall
CXXFLAGS_  = $(CXXFLAGS) $(OPTIMIZATION_FLAGS) -s DISABLE_EXCEPTION_CATCHING=0 -Wall -Isrc -I$(THIRDPARTY_DIR) -I$(GTEST_INC_DIR) -I$(GMP_INC_DIR)
LDFLAGS_ = $(LDFLAGS) $(OPTIMIZATION_FLAGS) -L. -L$(GTEST_LIB_DIR) -L$(GMP_LIB_DIR) -lgtest -lgmp -lgmpxx -lfte 
else
CFLAGS_ = $(CFLAGS) -Wall
CXXFLAGS_  = $(CXXFLAGS) -Wall -Isrc -I$(THIRDPARTY_DIR) -I$(GTEST_INC_DIR) -I$(GMP_INC_DIR)
LDFLAGS_ = $(LDFLAGS) -L. -L$(GTEST_LIB_DIR) -L$(GMP_LIB_DIR) -lgtest -lfte -lgmp -lgmpxx
endif
...
$(TARGET_MAIN): $(TARGET_LIBFTE) $(OBJ_MAIN)
	$(CXX) $(CXXFLAGS_) $(LDFLAGS_) $(OBJ_MAIN) -o $@
