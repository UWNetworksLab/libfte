...
class Encrypter {
 public:
  ...
  virtual bool Encrypt(const mpz_class & plaintext,
                       uint32_t plaintext_len_in_bits,
                       mpz_class * ciphertext) = 0;

  virtual bool Encrypt(const mpz_class & tweak,
  ...
  virtual bool Decrypt(const mpz_class & ciphertext,
                       uint32_t ciphertext_len_in_bits,
                       mpz_class * plaintext) = 0;

  virtual bool Decrypt(const mpz_class & tweak,
                       uint32_t tweak_len_in_bits,
